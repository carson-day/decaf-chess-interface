...
class ScalePanel extends JPanel implements ActionListener, IntegerChecker,
			...
			if (string == null) {
				...
				try {
					...
					if (!checker.allow(incoming)) {
						throw new NumberFormatException(incoming
								+ " not allowed!");
					}
	...
	private void insertAfter() {
		int newRowIndex = scaleTableModel.insertAfter(scaleTable
				.getSelectedRow());
		scaleTable.getSelectionModel().setSelectionInterval(newRowIndex,
				newRowIndex);
	}

	private void insertBefore() {
		int newRowIndex = scaleTableModel.insertBefore(scaleTable
				.getSelectedRow(), Integer.parseInt(startAt.getText()));
		scaleTable.getSelectionModel().setSelectionInterval(newRowIndex,
				newRowIndex);
	}
	...
	private void removeSelected() {
		...
		if (size > 1) {
			...
			if (row != -1) {
				...
				if (row < size) {
					scaleTable.getSelectionModel().setSelectionInterval(row,
							row);
				} else {
					scaleTable.getSelectionModel().setSelectionInterval(
							size - 1, size - 1);
				}
			}
		}
...
class ScaleTableModel extends AbstractTableModel {
	...
	public int insertAfter(int row) {
		...
		if (row != -1) {
			...
			ScaleData newRange = new ScaleData(current.start
					+ (next.start - current.start) / 2, 1);
			newRowIndex = row + 1;
	...
	public int insertBefore(int row, int start) {
		...
		if (row != -1) {
			...
			ScaleData newRange = new ScaleData(prev.start
					+ (current.start - prev.start) / 2, 1);
			newRowIndex = row;
			data.add(newRowIndex, newRange);
