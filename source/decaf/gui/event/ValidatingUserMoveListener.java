...
public class ValidatingUserMoveListener implements UserMoveInputListener {
	...
	private void fillBughouseDropMenu(final JPopupMenu popupMenu,
		...
		if (controller.isBughouse()) {
			...
			} else if (Coordinates.RANK_1 != source.getRank()
					&& Coordinates.RANK_8 != source.getRank()
	...
	private boolean isPromotion(Position position, int[] startCoordinates,
		...
		return (position.isWhitesMove()
				&& endCoordinates[0] == Coordinates.RANK_8 && position
				.get(startCoordinates) == Piece.WHITE_PAWN)
				|| (!position.isWhitesMove()
						&& endCoordinates[0] == Coordinates.RANK_1 && position
		...
		if (!wasSuccessful && isClickClickDND) {
			if (clickClickMoveStartSquare == null) {
				...
				if (clickClickMoveStartSquare == square) {
					...
					if ((PieceUtil.isWhitePiece(clickClickMoveStartSquare
							.getChessPiece()) && PieceUtil.isWhitePiece(square
							.getChessPiece()))
							|| (PieceUtil
									.isBlackPiece(clickClickMoveStartSquare
											.getChessPiece()) && PieceUtil
									.isBlackPiece(square.getChessPiece()))) {
						controller.getChessArea().getBoard()
								.unselectAllSquares();
						clickClickMoveStartSquare = square;
						clickClickMoveStartSquare.select();
					} else {
						controller.getChessArea().getBoard()
								.unselectAllSquares();
						makeMove(clickClickMoveStartSquare.getCoordinates(),
								square.getCoordinates(),
								clickClickMoveStartSquare.getDropPiece(),
								clickClickMoveStartSquare.isDropSquare());
					}
				}
			}
		}
