...
public class ChatTab extends JPanel implements ClipboardOwner {
	...
	private void addPopupActions(JPopupMenu menu, String propertiesName,
			final int obsGameNumber, final String person) {
		...
		if (obsGameNumber != -1) {
			...
			while (currentText != null || isSeperator != null
				...
				if (currentText != null) {
					...
					buildPersonPopupSubMenu(subMenu, person, newMenu,
							propertiesName);
					menu.add(newMenu);
				}
	...
	private void buildPersonPopupSubMenu(String menuName, String person,
		...
		String isSeperator = ResourceManagerFactory.getManager()
				.getString(propertiesName,
						...
						if (soughtCommand != null) {
							...
							if (quotedText != null) {
								...
								if (isPerson || gameNumber != -1) {
									addPopupActions(menu, "PersonPopup",
											gameNumber, isPerson ? word : null);
								} else {
									addPopupActions(menu, "RightClickPopup",
											-1, null);
								}
