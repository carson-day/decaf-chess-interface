...
public class Style12EventParser extends InboundEventParser {
	public Style12EventParser(Object source) {
		super(source, true, false);
		droppableParser = new DroppableHoldingsChangedEventParser(source);
	}
	...
	public InboundEvent parseInsideGameStart(String text) {
		return parse(false, text);
	}

	private InboundEvent parse(boolean onlyParseIfNoGameStart, String text) {
		if (text.length() < 600) {
			int style12Index = text.indexOf(IDENTIFIER);
			if (style12Index != -1) {

				if (onlyParseIfNoGameStart) {
					int gameStartIndex = text.indexOf(GAME_START_IDENTIFIER);
					if (gameStartIndex != -1) {
						return null;
					}
				}
				text = text.substring(style12Index);

				String positionString = text.substring(POSITION_START_INDEX,
						POSITION_END_INDEX);
				...
				int whiteRemainingTime = Integer.parseInt(tok.nextToken());
				int blackRemainingTime = Integer.parseInt(tok.nextToken());
				int standardChessMoveNumber = Integer.parseInt(tok.nextToken());
				String verboseNotationString = tok.nextToken();
				int timeTakenForLastMove = timeTakenStringToInt(tok.nextToken());
				...
				DroppableHoldingsChangedEvent holdingsChangedEvent = (DroppableHoldingsChangedEvent) droppableParser
						.parseInsideStyle12(text);

				Position position = new Position(positionArray,
						canWhiteCastleShort, canWhiteCastleLong,
						canBlackCastleShort, canBlackCastleLong,
						doublePawnPushFile, isWhitesMove);

				return new MoveEvent(getSource(), "" + id++, text, gameId,
						position, numberOfMovesSinceLastIrreversible,
						whitesName, blacksName, relation, initialTime,
						initialInc, whiteStrength, blackStrength,
						whiteRemainingTime, blackRemainingTime,
						standardChessMoveNumber, verboseNotationString,
						timeTakenForLastMove, prettyNotationString,
						isWhiteOnTop, isClockTicking, lagInMillis,
	...
	/**
	 * Parses a string in (0:06) format into an int.
	 */
	private static int timeTakenStringToInt(String timeTakenString) {
	...
	private static final String DROP_IDENTIFIER = "<b1>";

	private static final String IDENTIFIER = "<12>";

	private static final String OBSERVING_ID = "You are now observing game ";

	private static final String GAME_START_IDENTIFIER = "Creating: ";

	private static final int POSITION_START_INDEX = IDENTIFIER.length() + 1;

	private static final int POSITION_END_INDEX = POSITION_START_INDEX + 64 + 7;

	private DroppableHoldingsChangedEventParser droppableParser;
