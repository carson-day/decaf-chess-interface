...
public class ShoutEventParser extends InboundEventParser {
	public ShoutEventParser(Object source) {
	...
	public InboundEvent parse(String text) {
		if (text.length() < 1500) {
			if (text.startsWith(SHOUT_1)) {
				...
				return new ShoutEvent(getSource(), "" + id++, text, ParserUtil
						.removeTitles(s1));
			...
			if (stringtokenizer1.hasMoreTokens()) {
				...
				if (stringtokenizer1.hasMoreTokens()) {
					...
					if (s3.equals(SHOUT_2))
						return new ShoutEvent(getSource(), "" + id++, text,
								ParserUtil.removeTitles(s2));
				}
			}
			return null;
		}
