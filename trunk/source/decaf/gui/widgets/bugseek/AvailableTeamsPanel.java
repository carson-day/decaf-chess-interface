...
public class AvailableTeamsPanel extends JPanel {
	...
	public void setTeams(List<BugWhoPTeam> teams) {
		synchronized (this) {
			...
			else
			{
				...
				JLabel label = new JLabel("No available teams.",JLabel.CENTER);
	...
	{
		JLabel result = new JLabel(StringUtility.trimOrRightPad(text,chars),JLabel.LEFT);
		result.setFont(GUIManager.getInstance().getPreferences().getBoardPreferences().getControlLabelTextProperties().getFont());
		result.setBackground(GUIManager.getInstance().getPreferences().getBoardPreferences().getBackgroundControlsColor());
		result.setForeground(GUIManager.getInstance().getPreferences().getBoardPreferences().getControlLabelTextProperties().getForeground());
		return result;
	}
	...
	private boolean isTeamActive(BugWhoPTeam team)
	{
		...
		return (team.getPlayer1Modifier() == 0 || team.getPlayer1Modifier() == '.') &&
		       ...
		       !ParserUtil.removeTitles(team.getPlayer1Handle()).equals(User.getInstance().getHandle()) &&
		       !ParserUtil.removeTitles(team.getPlayer2Handle()).equals(User.getInstance().getHandle());
		      
	}
	
	private JPanel buildButtonPanel(final boolean isTeam1,final BugWhoPTeam team)
	{
		JPanel panel = new JPanel();
		panel.setLayout(new FlowLayout(isTeam1 ? FlowLayout.RIGHT : FlowLayout.LEFT));
