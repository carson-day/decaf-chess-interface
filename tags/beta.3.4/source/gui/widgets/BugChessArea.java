...
public class BugChessArea extends JPanel implements Preferenceable, Disposable {
	...
	private int orientation = AREA1_ON_LEFT;

	public List<JButton> ptellButtons = new LinkedList<JButton>();
	...
	public BugChessArea(ChessArea chessArea, ChessArea partnersChessArea) {
		...
		boardDividingSplitPane.setOneTouchExpandable(false);
		setupLayout();
	}
	...
	public void rotate() {
		synchronized (this) {
			...
			boardDividingSplitPane.setLeftComponent(isArea1OnLeft ? area2
					: area1);
			boardDividingSplitPane.setRightComponent(isArea1OnLeft ? area1
					: area2);
	...
	public void setDividerLocation() {
		int location = preferences.getBughousePreferences()
				.getBoardSplitterLocation();
		if (location == -1) {
			boardDividingSplitPane.setDividerLocation(600);
		} else {
