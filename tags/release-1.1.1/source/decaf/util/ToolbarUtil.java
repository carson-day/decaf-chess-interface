					...
					public void actionPerformed(ActionEvent e) {
						if (finalCommand.equalsIgnoreCase("$FLIP")) {
							...
							GUIManager.getInstance().removeController(
									controller);
						} else if (finalCommand.equalsIgnoreCase("$MATCH_WINNER")){
							...
							{							
								final GameNotificationListener listener = new GameNotificationListener() {
									...
									public void fire(ChessAreaControllerBase controller)
									{
										if (clicks > 0)
										{
											return;
										...
										try
										{
										   //sleep 100 so the controller can update with the games result.
										   Thread.sleep(100);
										}
										...
										int control = controller.getInitialTimeSecs()/60;
										int inc = controller.getInitialIncSecs();
										
										ChessArea chessArea = controller.getChessArea();
										
										if (controller.getGameEndState() == ChessAreaControllerBase.WHITE_WON)
										{
											playerName = ParserUtil.removeTitles(chessArea.getWhiteName());
											
											if (controller.isBughouse())
											{
												playerName2 = ParserUtil.removeTitles(controller.getPartnersChessArea().getBlackName());
											}
										}
										else if (controller.getGameEndState() == ChessAreaControllerBase.BLACK_WON)
										{
											playerName = ParserUtil.removeTitles(chessArea.getBlackName());
											if (controller.isBughouse())
											{
												playerName2 = ParserUtil.removeTitles(controller.getPartnersChessArea().getWhiteName());
											}
										...
										String matchString = "match " + playerName + " " + control + " " + inc;
										...
										EventService.getInstance().publish(new OutboundEvent(matchString));
										
										if (controller.isBughouse())
										{
											String matchString2 = "match " + playerName2 + " " + control + " " + inc + " bughouse";
											EventService.getInstance().publish(new OutboundEvent(matchString2));
								...
								GUIManager.getInstance().addGameNotificationListener(listener);
